...
### TODO write up these notes

* each component runs in a separate subshell
* pipefail setting
* caution against overly long pipelines
    * cost, subshells, spawning commands.
    * combinations of grep|cut|sed|awk can often be combined into a single awk or perl call.
    * style: line continuations
* other languages that borrowed pipelines: jq, F#
...
* In the first one, the if statement, C will only execute if A fails.
* In the second one, C executes if A fails _or if A succeeds but B fails_!
~~~~
...
```bash
[[ $word != "$candLower" ]] && [[ $key == "$(sorted "$candLower")" ]] && anagrams+=("$candidate")
```
...
```bash
printf "%s\n" "$@" | bc --mathlib | sort --general-numeric-sort
```
